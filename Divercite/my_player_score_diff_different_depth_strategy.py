...
class MyPlayer(PlayerDivercite):
    ...
    def __init__(self, piece_type: str, name: str = "LCD_ZePequeno"):
        ...
        piecesVarianceHeuristic = PiecesVarianceHeuristic('sigmoid','diff')
        ...
        self._controller: StrategyController = StrategyController().add_strategy(
            OpeningMoveStrategy(True), 2).add_strategy(
                MinimaxTypeASearch(scoreHeuristic,4,LRUCache(1000)), 9).add_strategy(
                MinimaxTypeASearch(scoreHeuristic, 6, LRUCache(1500)))   
